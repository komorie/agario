...
public class OldRoom : GOSingleton<OldRoom>   
{
    ...
    public void InitRoom(S_RoomList packet) //처음에 접속했을 때, 이미 접속해 있던 플레이어들 목록 받아서 관리 딕셔너리에 추가함.
    {
        ...
        foreach (S_RoomList.Player p in packet.players)
        {
            ...
            {
                OldMyplayer mPlayer = Instantiate(myPlayerPrefab).GetComponent<OldMyplayer>();    
            ...
            {
                OldPlayer player = Instantiate(playerPrefab).GetComponent<OldPlayer>(); ;
    ...
    public void RecvEnterGame(S_BroadcastEnterGame p) //서버에게서 새로운 유저가 들어왔다는 패킷을 받는 경우 -> 일단 MyPlayer일리는 없음(고려 안함)
    {
        ...
        {
            OldPlayer player = Instantiate(playerPrefab).GetComponent<OldPlayer>(); //새 플레이어 추가
            player.PlayerId = p.playerId;
